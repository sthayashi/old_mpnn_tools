...
#   electronegativities[-2:] = [3.44, 3.44] # O2. Question: Different sources of electronegativity matters (mendeleev vs ptable.com)?

#   dipole_polarizabilities = np.empty(atomic_numbers.shape, dtype=float)
#   for i in range(dipole_polarizabilities.shape[0]):
#       dipole_polarizabilities[i] = mendeleev.element(int(atomic_numbers[i])).dipole_polarizability
#   dipole_polarizabilities[-2:] = [5.3, 5.3]

#   return np.vstack((molecule_memberships, atomic_numbers)).transpose()
#   return np.vstack((molecule_memberships, electronegativities)).transpose()
#   return np.vstack((molecule_memberships, electronegativities, dipole_polarizabilities)).transpose()
...
def make_edge_indices(organic_coords, o2_coords, include_intermolecular=False):
    """
...
#   for k in keys:
    for k in tqdm(keys):
        ...
        # edges
        edge_indices_i, edge_indices_j = make_edge_indices(mol_pos, o2_pos, False)
        ...
        # Node featuers x is a 2d array, with first column representing molecular membership (1 for organic, 2 for O2)
