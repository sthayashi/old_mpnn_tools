...
#   transfer_integrals = np.zeros(o2pos.shape[0])
  ...
  # print(transfer_integrals)
    ...
    result = parallel(delayed(calc_pair)(organic_molecule, molecule_homo_orbital, o2pos, O2_HOMO, i) for i in range(num_o2))
...
start = time.time()
parallel = Parallel(n_jobs=-1, backend='multiprocessing')
with h5py.File(filename, "a") as file:
    ...
    for molecule_refcode in keys:
        ...
        transfer_integrals = np.zeros(len(result), dtype=np.float32)
