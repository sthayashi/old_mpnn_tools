...
def make_data_list():
    with h5py.File("parsed_molecules.h5", "r") as input:
        ...
        for mol in mol_names:
            ...
            organic_pos = torch.tensor(training.get("molecule_pos")[:])
            o2_pos = training.get("o2_pos")[:][o2_good_inds]
            ...
            assert TI.shape[0] == o2_pos.shape[0]
            ...
            for i in range(TI.shape[0]):
                ...
                # NOTE SH: Is this log just for numerical stability?
                y = abs(y)
                y = torch.log10(y)
                data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y=y)
                pos = torch.vstack((organic_pos, torch.tensor(o2_pos[i])))
                data.pos = pos
