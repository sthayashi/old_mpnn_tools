...
def make_data_list():
    with h5py.File("parsed_molecules.h5", "r") as input:
        ...
        for mol in mol_names:
            ...
            organic_pos = training.get("molecule_pos")[:]
            o2_pos = training.get("o2_pos")[:]
            ...
            for i in range(TI.shape[0]):
                ...
                # Which level should feature normalization be done? For each dimer? For each organic molecule? For entire dataset?
                ...
                pos = np.vstack((organic_pos, o2_pos[i])) # For cutoff
